// Armes à distance : pistolet de détresse (fusées éclairantes) et fusil-harpon
  ...
  return {
    ...
    update(dt, hooks) {
      for (let i = shots.length - 1; i >= 0; i--) {
        ...
        if (s.kind === 'harpoon') s.mesh.lookAt(s.pos.clone().add(s.vel));
        let done = false;
        const e = hooks.sweep(p0, s.pos);
        if (e) {
          if (s.owner === 'local') hooks.onHit(s.kind, e, s.vel.clone().setY(0));
          done = true;
