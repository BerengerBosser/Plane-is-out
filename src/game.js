...
export class Game {
  constructor() {
    ...
    this.bloom = new UnrealBloomPass(new THREE.Vector2(512, 512), 0.55, 0.45, 0.88);
  ...
  resetState() {
    ...
    this.own = { diable: false, wrench: false, lantern: false, flare: false, harpoon: false, talkie: false, mask: false, torch: false, rod: false };
    this.ammo = { flare: 0, harpoon: 0 };
  ...
  placeItem(it, x, z, rotY = it.rotY) {
    it.pos.set(x, this.groundAt(x, z, 99), z);
    it.rotY = rotY;
    it.state = 'ground';
    it.carrier = null;
  ...
  poseGround(it) {
    it.mesh.position.set(it.pos.x, this.groundAt(it.pos.x, it.pos.z, it.pos.y + 1) + it.rest - 0.05, it.pos.z);
    it.mesh.rotation.set(it.def.tilt || 0, it.rotY, 0, 'YXZ');
  }

  placeTools() {
    for (const [k, t] of Object.entries(this.tools)) {
      t.mesh.visible = !this.toolsTaken.has(k);
  ...
  nearIsland(p = this.playerWorld()) {
    const i2 = this.island2;
    return Math.hypot(p.x - i2.cx, p.z - i2.cz) < Math.hypot(p.x, p.z) ? 2 : 1;
  }

  // ── Objectifs ───────────────────────────────────────────
  chapter() { return this.flags.tookOff ? 2 : 1; }
  objectives() { return [...this.wreckObjectives(), ...this.baseObjectives()]; }
  baseObjectives() {
    const f = this.flags;
    if (this.chapter() === 1) {
      const list = [];
      list.push({ id: 'diable', text: 'Récupérer le diable de secours', hint: 'Dans les débris, sur la plage. Touche 3 pour s\'en servir.', done: this.own.diable || this.toolsTaken.has('diable') });
      list.push({ id: 'repair', text: `Réparer le Coucou (${this.installed.size}/6 pièces)`, hint: this.own.wrench ? '' : 'Une clé à molette accélère les réparations (plage sud)', done: this.installed.size === 6 });
      for (const k of PART_ORDER) list.push({ sub: true, text: ITEMS[k].name, hint: ITEMS[k].hint, done: this.installed.has(k) });
      list.push({ id: 'crate', text: 'Charger la caisse Hélios avec le treuil', hint: 'Treuil : porte cargo, côté droit de l\'avion. Portée 30 m.', done: this.crateLoaded });
      list.push({ id: 'takeoff', text: 'Monter à bord et décoller', hint: 'Porte cargo à droite, puis le siège pilote à l\'avant', done: f.tookOff });
      list.push({ id: 'chest', optional: true, text: `Bonus · ouvrir le coffre du canot (${this.symbols.size}/3 symboles)`, hint: 'Coffre au bout du ponton. Les symboles sont peints quelque part sur l\'île.', done: f.chest });
      list.push({ id: 'rod', optional: true, text: 'Bonus · s\'offrir une canne à pêche chez Jo', hint: 'Ramassez des coquillages sur les plages (3 🐚), caisse de troc au campement', done: !!this.own.rod });
      list.push({ id: 'king', optional: true, text: 'Bonus · vaincre le Crabe-Roi', hint: 'Sa crique est au nord-ouest. Attendez qu\'il charge : sa carapace s\'ouvre.', done: f.kingDead });
      return list;
    }
    const fz = (k) => this.fuses.has(k);
    return [
      { id: 'find', text: 'Trouver Saint-Escale au radar', hint: 'Le radar du tableau de bord montre l\'écho de l\'île', done: f.landed2 },
      { id: 'power', text: `Rétablir le courant (${this.fuses.size}/3 fusibles)`, hint: 'Tableau électrique de la centrale, à l\'ouest du terminal', done: f.power },
      { sub: true, text: 'Fusible rouge', hint: 'Terminal, bureau des objets trouvés', done: fz('red') },
      { sub: true, text: 'Fusible bleu', hint: 'Chariots à bagages sur le tarmac', done: fz('blue') },
      { sub: true, text: 'Fusible jaune', hint: 'Épave de l\'avion de ligne, au bout de la piste', done: fz('yellow') },
      { id: 'radio', text: 'Appeler Marthe depuis la tour de contrôle', hint: 'Ascenseur au pied de la tour (séquenceur de sécurité). La fréquence d\'urgence est sur la caisse Hélios.', done: f.radioDone },
      ...
      { id: 'wheels', text: 'Monter les roues sur les flotteurs', hint: 'Amenez l\'avion près du ponton', done: f.wheels },
      { id: 'fuel', text: 'Faire le plein au ponton', hint: this.nozzle !== 'plane' ? 'Décrochez le pistolet de la pompe et branchez-le sur l\'aile droite de l\'avion' : 'Panneau de la pompe : purgez le circuit, puis dosez la pression', done: f.refueled },
      { id: 'storm', text: 'Tenir la centrale jusqu\'à 21 h', hint: 'Tempête : la porte de l\'avion est bloquée. Défendez le générateur contre les zombies, réparez-le à la clé s\'il lâche', done: f.stormOver, hidden: !f.storm },
      { id: 'takeoff2', text: 'Rouler jusqu\'à la piste et décoller', hint: 'Remontez la rampe sur roues, puis plein gaz sur la piste', done: f.ended },
      { id: 'warden', optional: true, text: 'Bonus · vaincre le Colosse', hint: 'Il n\'est vulnérable que dans la lumière des projecteurs ou des fusées', done: f.wardenDead, hidden: !f.storm },
    ].filter((o) => !o.hidden);
  }
  currentObjective() {
  ...
  takePhoto() {
    ...
    try {
      ...
      const cap = txt ? txt.charAt(0).toUpperCase() + txt.slice(1) : `${this.nearIsland() === 2 ? 'Saint-Escale' : 'Plage du Crash'} · ${this.clock()}`;
  ...
  refreshCarnet() { this.ui.carnet(this.chapter() === 1 ? 'Chapitre 1 · Plage du Crash' : 'Chapitre 2 · Saint-Escale', this.objectives(), this.notes); }
  ...
  nextHint() {
    if (this.chapter() === 1) {
      if (!this.own.diable && !this.toolsTaken.has('diable')) return 'Le diable de secours est tombé près des débris, sur la plage. Sans lui, les moteurs vont vous casser le dos.';
      ...
      return 'Tout est prêt. Montez par la porte cargo, allez au siège pilote à l\'avant et mettez les gaz.';
    }
    const o = this.currentObjective();
  ...
  bindUI() {
    ...
    $('btnResume').addEventListener('click', () => { ui.show('pause', false); this.input.lock(); });
    ...
    bindRange('optRes', 'oRes', (v) => `${Math.round(v * 100)} %`, (v) => { this.resScale = v; this.onResize(); });
    ...
    this.canvas.addEventListener('click', () => {
      if (this.inGame() && !this.input.locked && !ui.modalOpen() && !ui.visible('endScreen') && !ui.visible('mapScreen')) { ui.show('pause', false); this.input.lock(); }
    });
    this.input.onLockChange = (locked) => {
      if (!locked && this.inGame() && !ui.modalOpen() && !this.input.dragMode && !this.chatting && !ui.visible('endScreen') && !ui.visible('settings') && !ui.visible('help') && !ui.visible('mapScreen')) {
        ui.show('pause', true);
        const n = this.session ? ` · équipage ${this.session.count()}` : '';
        document.getElementById('pauseInfo').textContent = `${this.chapter() === 1 ? 'Chapitre 1 · Plage du Crash' : 'Chapitre 2 · Saint-Escale'} · ${this.clock()} · canards ${this.ducks.size}/${this.duckSpots.length}${n}`;
        this.save();
      }
    };
  ...
  saveSettings() {
    ...
    store(SETTINGS_KEY, {
      ...
      bob: $('optBob').checked, shadows: $('optShadows').checked, bloom: $('optBloom').checked, fps: $('optFps').checked,
    });
  }
  ...
  saveData() {
    ...
    return {
      ...w, v: 5, savedAt: Date.now(), notes: this.notes, said: [...this.said], own: this.own, ammo: this.ammo, oil: this.oil, hp: this.hp, fish: this.fish,
  ...
  continueGame(fromSession = false) {
    ...
    Object.assign(this.own, s.own || {});
    Object.assign(this.ammo, s.ammo || {});
    this.oil = s.oil ?? 100;
    this.fish = { ...(s.fish || {}) };
  ...
  startIntro() {
    ...
    this.ui.subtitle('');
    this.ui.keys(this.isAuthority() ? '<kbd>Espace</kbd> passer la cinématique' : 'L\'hôte peut passer la cinématique');
  ...
  updateIntro(dt) {
    ...
    if (this.introAfter < 0) {
      ...
      if (u < 0.8) {
        ...
        const want = new THREE.Vector3(b.x + 24, heightAt(b.x + 24, b.z - 24) + 7, b.z - 24);
        this.camera.position.lerp(want, 1 - Math.exp(-2.5 * dt));
        this.camera.lookAt(p);
      }
      if (u >= 1) this.impact();
    } else {
  ...
  welcome() {
    this.radioOnce('hello', `Coucou, ici Marthe, labo Hélios. Vous me recevez ? … Dieu merci, vous êtes vivant${this.session && this.session.count() > 1 ? 's, tous' : 's'}. Réparez l'avion : la caisse doit absolument arriver jusqu'ici.`);
    this.radioOnce('watch', 'Et surveillez votre montre, touche T. À 19 h, la nuit tombe et les morts se relèvent. Le seul endroit sûr pour dormir : l\'avion, sur l\'eau. La carte est sur la touche M.');
  }
  ...
  loop(now) {
    ...
    if (this.bloomOn) {
      ...
      this.bloom.strength = 0.35 + (this.brumeNow || 0) * 0.55;
      this.composer.render();
    } else this.renderer.render(this.scene, this.camera);
    if (this.wantPhoto) this.takePhoto();
    if (this.mode === 'explore' && !this.lying && !this.downed) this.vm.render(this.renderer);
    this.input.endFrame();
    requestAnimationFrame((t) => this.loop(t));
  }
  ...
  update(dt) {
    ...
    if (inp.hit('F1')) this.ui.show('debug', this.ui.el.debug.classList.contains('hidden'));
    ...
    const sk = this.sky.update(hour, this.camera.position, this.t, dt, focus);
    ...
    if (this.inGame() || this.mode === 'dead') {
      if (this.isAuthority()) {
        this.enemies.update(dt, { players: this.enemyTargets(), night: sk.brume, depth: this.nightDepth(), lights: this.lightSources(), maxVoiles: CFG.combat.maxVoiles + 2 * (this.playerCount() - 1), siege: this.siege.active ? { pos: this.island2.points.generator, active: true } : null });
  ...
  secondsPerHour() { return 120 * [1.35, 1.15, 1, 1][Math.min(3, this.playerCount() - 1)]; }

  advanceTime(dt) {
    if (this.isAuthority()) {
      if (this.input.hit('F2')) { this.fast = !this.fast; this.ui.toast('Temps', this.fast ? 'Accéléré ×10' : 'Normal'); }
      if (this.input.hit('F3')) { this.hour = CFG.time.alarmHour - 0.02; this.flags.alarm = false; }
  ...
  lanternOn() { return this.mode === 'explore' && !this.aboard && !this.downed && this.slot === 3 && this.own.lantern && this.oil > 0; }
  ...
  hurt(dmg, type) {
    ...
    this.hp -= dmg * (this.own.mask ? 0.65 : 1);
  ...
  planeColliders() {
    ...
    if (this.installed.has('floats')) {
      for (const sx of [-2.3, 2.3]) for (const z of [-3.6, -1.8, 0, 1.8, 3.6]) {
        const w = root.localToWorld(new THREE.Vector3(sx, 0, z));
        cs.push({ type: 'circle', x: w.x, z: w.z, r: 0.6, minY: root.position.y - 2.5, maxY: root.position.y + 2 });
      }
    }
    return cs;
  }
  ...
  // ── Barre d'outils ──────────────────────────────────────
  ownedSlots() { return [true, this.own.wrench, this.own.diable, this.own.lantern, this.own.flare, this.own.harpoon, this.own.rod]; }
  selectSlot(i) {
    const names = ['Poings', 'Clé à molette', 'Diable', 'Lanterne', 'Pistolet de détresse', 'Fusil-harpon', 'Canne à pêche'];
    if (!this.ownedSlots()[i]) { this.ui.toast(names[i], 'Pas encore trouvé.', 'bad', 1800); return; }
    if (this.carrying && this.carryMode === 'diable' && i !== 2) { this.ui.toast('Mains prises', 'Lâchez d\'abord la pièce (<kbd>G</kbd>).', 'bad', 2000); return; }
    if (this.carrying && this.carryMode === 'hand' && i === 2) { this.ui.toast('Mains prises', 'Lâchez la pièce (<kbd>G</kbd>) puis rechargez-la sur le diable.', 'bad', 2200); return; }
    this.slot = i;
  }
  ...
  updateExplore(dt) {
    ...
    if (this.planeLive && this.ownsPlane()) {
      ...
      if (ev.includes('bump')) this.audio.clank();
    }
    if (this.planeLive) this.audio.setEngine(this.flight.airborne ? 0.4 + this.flight.throttle * 0.4 : 0, this.flight.throttle * 0.8);

    if (!this.aboard && !this.chatting) {
      ['Digit1', 'Digit2', 'Digit3', 'Digit4', 'Digit5', 'Digit6', 'Digit7'].forEach((c, i) => { if (inp.hit(c)) this.selectSlot(i); });
      if (inp.hit('WheelDown') || inp.hit('WheelUp')) {
        const owned = this.ownedSlots();
        const d = inp.hit('WheelDown') ? 1 : -1;
        for (let k = 1; k <= 7; k++) { const i = (this.slot + d * k + 14) % 7; if (owned[i]) { if (i !== this.slot) this.selectSlot(i); break; } }
      }
    }
    ui.hotbar(this.slot, this.ownedSlots(), this.oil);
    ui.ammo(this.ammo.flare, this.ammo.harpoon);
    const fc = this.fishCount(); if (fc !== this._fc) { this._fc = fc; document.getElementById('fishCount').textContent = fc; }
    ...
    // roue de messages rapides (C maintenu, en équipe)
    const wheelOn = !!this.session && inp.down('KeyC') && !carnetOpen && !this.chatting && !ui.modalOpen();
    if (wheelOn) {
    ...
    const blocked = carnetOpen || ui.modalOpen() || this.chatting || ui.visible('mapScreen') || wheelOn;
    ...
    if (this.downed) {
      ...
      const mods = {
        ...
        canJump: w <= 1 && !swim,
        speedMul: (w ? Math.min(0.95, speedTable[w] * (help ? 2.6 : 1)) : 1) * (swim ? CFG.swim.speed : 1),
      };
      this.player.radius = this.aboard ? 0.24 : undefined;
      if (this.aboard) mv = this.player.update(dt, inp, cabinColliders(this.crateLoaded, this.upgrades), mods, this.cabinEnv());
      else mv = this.player.update(dt, inp, this.colliders.concat(this.planeColliders()), mods, this.worldEnv());
    ...
    if (this.aboard && !this.seat && this.player.pos.x > 1.55 && !this.flight.airborne) this.leavePlane();
    this.updatePlaneRepair(dt);
    this.poseCarried();
    if (this.isAuthority()) this.updateSliding(dt);
    if (!blocked && !this.downed) this.handleInteractions(dt);
    else { ui.prompt(''); ui.hold(0); }
    ...
    if (this.own.harpoon && !this.aboard) {
      const n = this.projectiles.collect(this.player.pos);
      if (n) { this.ammo.harpoon = Math.min(8, this.ammo.harpoon + n); this.audio.pickup(); }
    }
    ...
    const hints = document.getElementById('optHints').checked;
    let keys = '';
    if (hints) {
      if (this.seat) keys = '<kbd>E</kbd> se lever';
      else if (this.lying) keys = '<kbd>E</kbd> se lever du hamac';
      else if (this.aboard) keys = '<kbd>Tab</kbd> carnet · <kbd>M</kbd> carte<br><kbd>E</kbd> interagir';
      else keys = `<kbd>Tab</kbd> carnet · <kbd>M</kbd> carte · <kbd>T</kbd> montre<br><kbd>E</kbd> interagir · <kbd>G</kbd> lâcher<br><kbd>1</kbd>–<kbd>7</kbd> outils · <kbd>Clic</kbd> frapper / tirer / pêcher${this.session ? '<br><kbd>Entrée</kbd> message · <kbd>B</kbd> parler · <kbd>V</kbd> repère' : ''}`;
    }
    ui.keys(keys);
    ui.flight(false);
    ...
    this.vm.update(dt, {
      ...
      hidden: carnetOpen || this.lying, mdx: inp.mdx, mdy: inp.mdy, ammo: this.slot === 4 ? this.ammo.flare : this.ammo.harpoon, reeling: this.fishState === 'reel', rodPitch: this.fishState === 'reel' ? -0.3 : this.fishState === 'wait' || this.fishState === 'bite' ? -0.15 : 0,
    });
    ...
    if (this.chapter() === 2 && !this.flags.landed2 && !this.aboard && this.nearIsland() === 2 && heightAt(this.player.pos.x, this.player.pos.z) > 0) {
      ...
      this.radioOnce('landed2', 'Vous y êtes ! Saint-Escale. Sans courant, rien ne marche là-bas : la centrale est à l\'ouest du terminal, il lui faut trois fusibles.');
    }
    if (this.t - this.lastProgress > 150 && !this.flags.ended) { this.lastProgress = this.t; this.ui.radio(this.nextHint(), () => this.audio.radio()); }
    if (inp.hit('F4')) this.debugRepairAll();
  }
  ...
  debugRepairAll() {
    ...
    ['diable', 'wrench', 'lantern'].forEach((k) => { if (!this.toolsTaken.has(k)) this.act('tool', { k, silent: true }); this.own[k] = true; });
  ...
  updateRadar(dt) {
    ...
    if (!this.flags.discovered && this.chapter() === 2 && Math.hypot(f.pos.x - I.cx, f.pos.z - I.cz) < 700) {
      ...
      this.radioOnce('discovered', 'Je capte votre balise près de Saint-Escale ! Amerrissez dans la baie au sud, près de la rampe et du ponton.');
    }
  ...
  updateTracker() {
    const island = this.nearIsland() === 2 ? 'Île 2 · Saint-Escale' : 'Île 1 · Plage du Crash';
    const o = this.currentObjective();
    let sub = o.hint || '';
    if (o.id === 'repair') {
  ...
  toggleMap(force) {
    ...
    if (on && !this.mapBuilt) { this.map.build(this.island2.cx, this.island2.cz); this.mapBuilt = true; }
  ...
  // ── Vol ─────────────────────────────────────────────────
  parkAt(island) {
    if (island === 2) { const p = this.island2.points.park; this.flight.reset(p.x, p.z, I2.park.yaw); }
  ...
  updateFlight(dt) {
    ...
    for (const e of ev) {
      if (e === 'takeoff') {
        ...
        if (!this.flags.tookOff) {
          ...
          this.audio.success();
          ui.toast('Le Coucou vole !', 'Chapitre 2 : trouver l\'aéroport de Saint-Escale. Suivez l\'écho rouge du radar.', 'good', 9000);
          this.radioOnce('takeoff', `Je vous vois ! Votre radar doit capter un écho au cap ${this.bearingTo(f.pos.x, f.pos.z, this.island2.cx, this.island2.cz)}°. C'est sûrement Saint-Escale. Surveillez le carburant.`);
        }
        if (wasGround && this.chapter() === 2 && this.nearIsland(f.pos) === 2 && !this.flags.ended) this.endDemo();
      ...
      else if (e === 'ap_on') ui.toast('Pilote automatique', 'Cap et altitude tenus. <kbd>E</kbd> pour se lever et marcher dans la cabine.', 'good');
      ...
      else if (e === 'bump') { this.audio.clank(); ui.toast('Choc', 'L\'avion bute contre un obstacle.', 'bad', 1500); }
    }
    ...
    let state = f.onWater ? (f.speed < 1 ? 'À l\'arrêt sur l\'eau' : f.speed >= CFG.flight.takeoffSpeed ? 'Vitesse de décollage : tirez !' : 'Sur l\'eau')
      : f.surface === 'ground' ? (f.noTakeoff ? 'Tempête : décollage bloqué' : f.speed >= CFG.flight.takeoffSpeed ? 'Vitesse de décollage : tirez !' : 'Au sol, sur roues')
    ...
    ui.flight(true, { speed: f.speed, alt: f.surface === 'ground' ? 0 : f.pos.y, throttle: f.throttle, fuel: f.fuel, state });
    ui.keys(document.getElementById('optHints').checked ? '<kbd>Z</kbd>/<kbd>S</kbd> gaz · <kbd>Q</kbd>/<kbd>D</kbd> palonnier<br>Souris ou flèches : manche<br><kbd>P</kbd> pilote auto · <kbd>C</kbd> vue · <kbd>M</kbd> carte<br><kbd>E</kbd> quitter le siège' : '');
  ...
  showEnd() {
    ...
    document.getElementById('endStats').innerHTML = [
      ...
      [`${this.ducks.size}/${this.duckSpots.length}`, 'canards'], [`${[this.flags.kingDead, this.flags.wardenDead].filter(Boolean).length}/2`, 'boss vaincus'],
    ].map(([b, s]) => `<li><b>${b}</b><span>${s}</span></li>`).join('');
    setTimeout(() => {
      this.input.unlock();
      this.ui.show('endScreen', true);
      this.ui.radio('Vous avez réussi ! Cap à l\'ouest vers Kerloch. Et gardez la caisse bien au sec.', () => this.audio.radio());
    }, 2500);
  }
...
Object.assign(Game.prototype, WorldMixin, InteractMixin, MPMixin, CombatMixin, NightMixin, WreckMixin, FishingMixin, SavesMixin);
